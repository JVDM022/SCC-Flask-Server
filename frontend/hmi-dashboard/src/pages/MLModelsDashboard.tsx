...
export function MLModelsDashboard({ data }: MLModelsDashboardProps) {
  ...
  const residuals = data.history.map((row) => row.temp_c - row.setpoint_c);
  ...
  return (
    <main className="workspace">
      <section className="model-card-grid">
        {modelCards.map((model) => (
          <MetricCard
            ...
            detail={`RMSE ${formatNumber(model.rmse)} | MAE ${formatNumber(model.mae)} | Last -- | Dataset ${model.datasetSize ?? '--'}`}
          />
        ))}
      </section>
      ...
      <section className="analytics-grid">
        ...
        <TrendPanel title="Residual Analysis" series={[{ name: 'Temp error residual', x: data.history.map((row) => row.created_at || row.ms), y: residuals, color: '#F59E0B' }]} height={300} />
        <TrendPanel title="Prediction Accuracy Proxy" series={[{ name: 'Absolute temp error', x: data.history.map((row) => row.created_at || row.ms), y: residuals.map(Math.abs), color: '#22C55E' }]} height={300} />
        <section className="panel">
          <div className="panel__header">
            <div>
