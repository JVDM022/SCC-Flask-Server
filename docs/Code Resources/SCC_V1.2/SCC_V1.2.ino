...
// ================== SETPOINT ==================
#define SETPOINT_Cx100   13500   // 135.00 C
...
#define OFF_LOCKOUT_HIGH_Cx100 13520  // 135.20C
#define OFF_LOCKOUT_LOW_Cx100   13450  // 134.50C

// Relay autotune around setpoint
#define AUTOTUNE_WARMUP_Cx100   13000    // start relay phase near 130.00C
...
bool motorEnabled = false;     // SET_ON 1/0 controls this

uint32_t programStartMs = 0;
bool runtimeExpired = false;
bool manualKill = false;       // KILL 1/0 controls this
...
void handleCommand(char *line, uint32_t now, float tempC_now) {
  ...
  if (strcmp(cmd, "KILL") == 0) {
    if (arg == NULL) {
      Serial.println("ERR KILL requires 0 or 1");
      return;
    }
    ...
    if (val == 1) {
      ...
      motorEnabled = false;
      motorSet(false);
      Serial.println("OK KILL=1");
    } else if (val == 0) {
      manualKill = false;
      pidIntegral = 0.0f;
      pidLastTempC = tempC_now;
      pidLastMs = now;
      Serial.println("OK KILL=0");
    } else {
      Serial.println("ERR KILL must be 0 or 1");
    }
    return;
  }

  if (strcmp(cmd, "SET_ON") == 0) {
    if (arg == NULL) {
      Serial.println("ERR SET_ON requires 0 or 1");
      return;
    }
    ...
    if (val == 1) {
      motorEnabled = true;
      motorCycleStart = now;
      Serial.println("OK SET_ON=1");
    } else if (val == 0) {
      motorEnabled = false;
      motorSet(false);
      Serial.println("OK SET_ON=0");
    } else {
      Serial.println("ERR SET_ON must be 0 or 1");
    }
    return;
  }
  ...
  if (strcmp(cmd, "AUTOTUNE") == 0) {
    resetAutotune(now);
    Serial.println("OK AUTOTUNE");
    return;
  }

  if (strcmp(cmd, "STATUS") == 0) {
    Serial.print("STATUS T=");
    Serial.print(tempC_now, 2);
    Serial.print(",ON=");
...
void serviceSerial(uint32_t now, float tempC_now) {
  while (Serial.available() > 0) {
    ...
    if (c == '\n') {
      cmdBuf[cmdIdx] = '\0';
      if (cmdIdx > 0) {
        handleCommand(cmdBuf, now, tempC_now);
      }
      cmdIdx = 0;
      continue;
    }
...
void setup() {
  ...
  Serial.println("Boot: AUTOTUNE every reset -> then PID hold.");
  Serial.println("Commands:");
  Serial.println("  KILL 1");
  Serial.println("  KILL 0");
  Serial.println("  SET_ON 1");
  Serial.println("  SET_ON 0");
  Serial.println("  AUTOTUNE");
  Serial.println("Send each command with newline.");
}
